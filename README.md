...
the electric potential `epot` [kV] and field-aligned current `fac` [µA/m², + downward] as a (time, mlat, mlt) cube,
`NaN` equatorward of the auroral boundary and wherever a driver is missing. The drivers (`by`, `bz`, `vsw`, `nsw`,
`tilt`) are used as given (`run_weimer05(mlat, mlt, by=0., bz=-5., vsw=450., nsw=9.)`); the ones you leave out come from
the index store (NOAA OMNI) as the **mean of the previous 20 min of the 5-min series** (`res='5min', avg=20`, as in
Weimer 2005b), **not** instantaneous values — `avg=None, res='1min'` gives those. Also available in `survey` below.
...
**Driving the Weimer model from these series.** `models.weimer05` / `run_weimer05` use the previous 20 min of the
5-min series by default (`res='5min', avg=20`), as in Weimer (2005b). OMNI speed and density have gaps, which give NaN
rows unless you pass `nsw=` yourself. Recipes and details:
[`src/weimer05/README.md`](src/weimer05/README.md#driver-averaging).
...
#### `Empirical.run_weimer05` (spherical-cap-harmonic revision, Fortran translation by B. Foster, HAO/NCAR)
