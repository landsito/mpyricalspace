...
/*
gsl_multifit_ndlinear_getcoeff()
  Determine the coefficient corresponding to the set of indices
(i_1, i_2, ..., i_n) of the original problem

Inputs: indices - array of length w->n_dim containing coefficient
                  indices
        c       - coefficient vector as determined by gsl_multifit_linear
        coeff   - (output) where to store coefficient
        w       - workspace
...
int
gsl_multifit_ndlinear_getcoeff(const size_t *indices, const gsl_vector *c,
                               double *coeff,
                               gsl_multifit_ndlinear_workspace *w)
{
  if (c->size != w->n_coeffs)
  ...
  else
    {
      ...
      s = gsl_multifit_ndlinear_getidx(indices, &idx, w);
      if (s != GSL_SUCCESS)
        {
          GSL_ERROR("unable to find linear index", s);
        }

      *coeff = gsl_vector_get(c, idx);
...
/*
gsl_multifit_ndlinear_getidx()
  Determine the linear coefficient index corresponding to
the given multidimensional set of indices

The linear index is given by:

index = sum_{j=1}^{n} [ i_j * prod_{k=j+1}^{n} N_k ]

Inputs: indices - array of indices (length w->n_dim)
                  indices[j] = i_j in the documentation
        idx     - (output) where to store linear index
        w       - workspace
...
int
gsl_multifit_ndlinear_getidx(const size_t *indices, size_t *idx,
                             gsl_multifit_ndlinear_workspace *w)
{
  size_t index;
  size_t i;
