...
!
! Progress messages from read_potential and setmodel. On by default (the test program relies on them);
! weimer05_batch switches them off so a python session is not flooded.
!
  logical :: verbose = .true.
!
! The data files never change, so each reader remembers which file it last loaded and skips re-reading it
! (setmodel is called once per set of drivers, and used to re-read all of them every time).
!
  character(len=512) :: loaded_pot = '', loaded_scha = '', loaded_bndy = ''
!
  contains 
!-----------------------------------------------------------------------
...
!
  if (trim(infile) == trim(loaded_pot)) return
...
! enddo
  close(lu)
  if (verbose) write(6,"('read_potential: Closed file ',a)") infile
  loaded_pot = infile
...
!
  ...
  integer :: i,j,lu=20
!
  if (trim(infile) == trim(loaded_scha)) return
  open(lu,file=infile,status='old')
  read(lu,"(a)") fname
...
!
  if (trim(infile) == trim(loaded_bndy)) return
  open(lu,file=infile,status='old')
  read(lu,"(a)") fname
