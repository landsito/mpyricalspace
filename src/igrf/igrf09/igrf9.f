...
c     in July 2003 by IAGA Working Group V-8. 
C     It is the 9th generation IGRF, ie the 8th revision. 
C     The main-field models for 1900.0, 1905.0,..1940.0 are non-definitive,
C     those for 1945.0, 1950.0,...2000.0 are definitive and the
C     secular-variation model for 2000.0 to 2005.0 is non-definitive.
C
C     Main-field models are to degree and order 10 (ie 120 coefficients)
C     for 1900.0-1995.0 and to 13 (ie 195 coefficients) for 2000.0. The 
C     predictive secular-variation model is to degree and order 8 (ie 80
C     coefficients).
C
...
C
      ...
      WRITE(6,*)'* July 2003 by the IAGA Working Group V-8.           *'
      WRITE(6,*)'*                                                    *'
      WRITE(6,*)'* It is valid for dates from 1900.0 to 2005.0,       *'
      WRITE(6,*)'* values up to 2010.0 will be computed but with      *'
...
C
      CALL IGRF9SYN (0,DATE,ITYPE,ALT,CLT,XLN,X,Y,Z,F)
      D = FACT*ATAN2(Y,X)
      H = SQRT(X*X + Y*Y)
      S = FACT*ATAN2(Z,H)
...
C
      CALL IGRF9SYN (1,DATE,ITYPE,ALT,CLT,XLN,DX,DY,DZ,F1)
...
c
      implicit double precision (a-h,o-z)
      dimension gh(2865),g0(120),g1(120),g2(120),g3(120),g4(120),
     1          g5(120),g6(120),g7(120),g8(120),g9(120),ga(120),
     2          gb(120),gc(120),gd(120),ge(120),gf(120),gg(120),
     3          gi(120),gj(120),gk(195),gl(195),gm(195),p(105),q(105),
     4          cl(13),sl(13)
...
c
     ...
     e              1.,   -2.,    3.,    3.,    3.,   -1.,    0.,   -6./ 1990
      data gk/ -29692.,-1784., 5306.,-2200., 3070.,-2366., 1681., -412., 1995
     1           1335.,-2266., -262., 1248.,  302.,  758., -427.,  940., 1995
     2            780.,  262.,  290., -236., -418.,   97.,  122., -306., 1995
     3           -214.,  352.,   46.,  235.,  165., -118., -142., -166., 1995
     4            -55.,  -17.,  107.,   68.,   67.,  -17.,   68.,   72., 1995
     5           -170.,   67.,    0.,  -58.,   18.,    1.,  -93.,   36., 1995
     6             77.,  -72.,  -69.,    0.,  -25.,   28.,    4.,    5., 1995
     7             24.,    4.,   17.,    8.,  -24.,   -2.,   -6.,   25., 1995
     8              6.,   11.,   -6.,  -21.,   -9.,    8.,  -14.,  -23., 1995
     9              9.,   14.,    6.,   11.,   -4.,  -16.,   -7.,   -4., 1995
     a              4.,    9.,  -20.,    2.,   15.,  -10.,   12.,    8., 1995
     ...
     r              0.4,   -0.2,    0.0,   -0.5,    0.1,   -0.9/         2000
      data gm/    13.3,  11.6, -21.2, -14.4,  -3.7, -22.7,  -3.6, -11.1, 2002
     1            -1.1,  -3.5,   5.6,  -1.2,  -4.5,  -8.5,  -8.0,  -2.7, 2002
     2             2.2,   1.7,  -8.0,   1.0,   4.5,   5.1,  -1.9,  -0.3, 2002
     3            -1.4,   0.7,  -0.3,  -2.6,   1.5,  -1.2,   2.0,   0.0, 2002
     4             3.8,  -0.2,  -0.5,   0.4,   0.3,  -0.7,   0.7,  -1.8, 2002
     5             1.9,  -0.2,  -1.7,  -0.4,  -0.5,  -0.2,   0.7,   1.5, 2002
     6             0.2,  -0.1,   0.7,  -0.3,   0.3,   1.1,   0.1,   0.7, 2002
     7             0.3,   0.5,  -0.8,  -0.3,  -0.1,   0.5,   0.2,   0.1, 2002
     8             0.2,  -0.2,  -0.5,   0.1,   0.2,   0.3,  -0.4,   0.4, 2002
     9             0.2,   0.1,   0.5,  -0.3,  -0.7,   0.4,   0.4,   0.4, 2002
     a         115*0.0/                                                  2002
c
...
c
      ...
      y     = 0.0
      z     = 0.0
      if (date.lt.1900.0.or.date.gt.2010.0) go to 11
  ...
  960 format (/' This version of the IGRF is intended for use up',
     ...
     2        ' but may be of reduced accuracy'/)
      if (date.ge.2000.0) go to 1
      t     = 0.2*(date - 1900.0)                                             
      ll    = t
      one   = ll
      t     = t - one
      if (date.le.1995.0) then
       nmx   = 10
       nc    = nmx*(nmx+2)
       ll    = nc*ll
       kmx   = (nmx+1)*(nmx+2)/2
...
c
  ...
  961 format (/' This subroutine will not work with a date of',
     ...
     2        '.le.2010.0.  On return f = 1.0d8., x = y = z = 0.')
      return
      end